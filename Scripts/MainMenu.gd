...
func _ready():
	...
	how_to_play_button.button_down.connect(_on_how_to_play_button_down)
	how_to_play_button.button_up.connect(_on_how_to_play_button_up)
	
	# Aguardar um frame para o botão ter seu tamanho definido
	await get_tree().process_frame
	play_button.pivot_offset = play_button.size / 2
	how_to_play_button.pivot_offset = how_to_play_button.size / 2
...
func _on_how_to_play_button_down():
	var tween = create_tween()
	tween.tween_property(how_to_play_button, "scale", Vector2(0.9, 0.9), 0.1)

func _on_how_to_play_button_up():
	var tween = create_tween()
	tween.tween_property(how_to_play_button, "scale", Vector2(1.0, 1.0), 0.1)
...
func _on_how_to_play_button_pressed():
	await get_tree().create_timer(0.15).timeout
	get_tree().change_scene_to_file("res://Scenes/como_jogar_2.tscn")
